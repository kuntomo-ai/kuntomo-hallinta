...
export default function RaportointiValmennus() {
  ...
  async function fetchData() {
    ...
    if (!from || !to) return
    setLoading(true)
    const { data } = await supabase.from('valmennusmyynti').select('*').gte('created_at', from).lte('created_at', to + 'T23:59:59').order('created_at', { ascending: false })
    setRows(data || [])
    setLoading(false)
  }
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">Valmennuspalveluiden myyntiraportti</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '1.5rem', alignItems: 'start' }}>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Pvm</th><th>Asiakas</th><th>Palvelu</th><th>Hinta</th><th>Maksutapa</th></tr>
            </thead>
            <tbody>
              {loading ? (
